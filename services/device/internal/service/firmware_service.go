...
func (s *firmwareService) SignFirmware(ctx context.Context, releaseID uint, privateKeyPEM string) error {
	...
	// Sign the hash
	r, signatureS, err := ecdsa.Sign(rand.Reader, privateKey, hash[:])
	if err != nil {
		return fmt.Errorf("failed to sign firmware: %w", err)
	}

	// Format the signature
	signature := fmt.Sprintf("%x,%x", r, signatureS)
...
func (s *firmwareService) GetFirmwareFile(ctx context.Context, releaseID uint) (string, io.ReadCloser, error) {
	...
	// Create a wrapper to ensure file is closed if the caller doesn't
	safeReader := &safeReadCloser{
		ReadCloser: file,
		onClose: func() {
			s.log.Debug("Firmware file closed: ", release.FilePath)
		},
	}

	return filepath.Base(release.FilePath), safeReader, nil
}
...
func (s *firmwareService) verifySignature(release *models.FirmwareReleaseExtended) bool {
	...
	signatureS, ok := new(big.Int).SetString(sHex, 16)
	if !ok {
	...
	return ecdsa.Verify(publicKey, hash[:], r, signatureS)
}
...
// Add a safe read closer implementation
